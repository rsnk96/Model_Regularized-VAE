...
parser.add_argument('--checkpoint_path', default='output/models/iter_020679/')
parser.add_argument('--aux_data', default='output/models/aux_data.pkl')
parser.add_argument('--batch_size', default=-1, type=int) # -1 implies it will use train batch size itself 
parser.add_argument('--dump_path', default='output/dump')
...
for im in range(30):
    ...
    reshaped_image = reshaped_image.reshape((84, 84, 3))
...
for i in range(n_test_batches) :
	z,v = VAE.get_conditional_params(sess,np.reshape(sample_batch(frames, perm_valid, n_test_imgs_per_batch),(-1, 84, 84, 3)))
	encoded.append(z)
	cvars.append(v)
...
vis_images_encoded = VAE.encode_ML(sess,np.reshape(sample_batch(frames, perm_valid, n_vis_images),(-1, 84, 84, 3)))
...
for i in range(n_vis_images):
    ...
    for l in range(6) :
        ...
        for delta in sd_shifts :
            ...
            im_l.append(np.reshape(VAE.decode(sess,[z_new]),(84,84,3)))
