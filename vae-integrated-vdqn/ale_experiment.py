...
class ALEExperiment(object):
    def __init__(self, ale, agent, resized_width, resized_height,
        ...
        self.screen_buffer = np.empty((self.buffer_length,
                                      ...
                                      dtype=np.uint8)
        self.screen_buffer_color = np.zeros((self.buffer_length,
        ...
        self.VAE_data_set = ale_data_set.RawDataSet(width=self.width,
                                             height=self.height,
                                             rng=rng,
                                             max_steps=self.agent.phi_length*2,
    ...
    def _init_episode(self):
        ...
        self._act(0)
        self._act(0)
        if self.agent.phi_length > 2 :
            for i in range(self.agent.phi_length-2) :
                self._act(0)
    ...
    def get_observation(self):
        ...
        lastphi = self.VAE_data_set.last_phi()
        obs_encoded = self.VAE.encode_ML(self.sess, [lastphi])
        return obs_encoded.reshape((1, obs_encoded.size))
